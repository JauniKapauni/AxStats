...
public class DatabaseManager {
    ...
    public boolean initDatabaseTable1(){
        try(Connection conn = getConnection()){
            try(PreparedStatement ps = conn.prepareStatement("CREATE TABLE IF NOT EXISTS players (uuid VARCHAR(255) NOT NULL PRIMARY KEY, deaths INT, sessions INT, isOnline BOOLEAN)")){
                ps.executeUpdate();
                return true;
            }
